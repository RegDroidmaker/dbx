...
class XBoxController:
    ...
    # --- NOVA FUNÇÃO: ZONA MORTA (Corrige o Drift) ---
    def apply_deadzone(self, value, threshold=0.12):
        """Se o valor for menor que o threshold (12%), retorna 0."""
    ...
    def get_commands(self):
        ...
        # --- LEITURA E TRATAMENTO DOS ANALÓGICOS ---
        
        # 1. Leitura Bruta (PS4: Joy Esq = 0 e 1, Joy Dir = 3 e 4)
        raw_l_x = -1 * self.p1.get_axis(0)
        raw_l_y = -1 * self.p1.get_axis(1)
        raw_r_x = -1 * self.p1.get_axis(3)
        raw_r_y = -1 * self.p1.get_axis(4)

        # 2. Aplicação da Deadzone (Remove Drift)
        l_x = self.apply_deadzone(raw_l_x)
        l_y = self.apply_deadzone(raw_l_y)
        r_x = self.apply_deadzone(raw_r_x)
        r_y = self.apply_deadzone(raw_r_y)

        # --- LEITURA E TRATAMENTO DOS GATILHOS (ANTENAS) ---
        
        # L2 (Eixo 2) - Arredondado para 2 casas decimais para reduzir ruído
        left_trigger_raw = (self.p1.get_axis(2) + 1) / 2
        left_trigger = np.around(left_trigger_raw, 2) 

        # R2 (Eixo 5) - Arredondado para 2 casas decimais para reduzir ruído
        right_trigger_raw = (self.p1.get_axis(5) + 1) / 2
        right_trigger = np.around(right_trigger_raw, 2) 
